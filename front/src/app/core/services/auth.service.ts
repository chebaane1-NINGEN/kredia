...
export type AppRole = 'ADMIN' | 'AGENT' | 'CLIENT' | 'SUPER_ADMIN';
...
export class AuthService {
  ...
  getCurrentUserRole(): AppRole | null {
    const role = this.decodeToken()?.role?.toUpperCase();
    if (role === 'ADMIN' || role === 'AGENT' || role === 'CLIENT' || role === 'SUPER_ADMIN') {
      return role;
    }
    return null;
  }
  ...
  isSupportStaff(): boolean {
    return this.hasAnyRole('ADMIN', 'AGENT', 'SUPER_ADMIN');
  }
